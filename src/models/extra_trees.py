...
class ExtraTreesModel(BaseModel):
    """Concrete implementation of BaseModel using scikit-learn ExtraTreesClassifier."""
    ...
    def train(self, X: pd.DataFrame, y: pd.Series) -> None:
        """Trains the Extra Trees classifier on features X and labels y."""
        logger.info(f"Training ExtraTreesModel: Samples={len(X)}, Features={X.shape[1]}")
        self._feature_names = list(X.columns)
        self.model.fit(X, y)
        self.is_trained = True
        logger.info("ExtraTreesModel training completed successfully.")
        
    def predict(self, X: pd.DataFrame) -> np.ndarray:
        """Predicts binary price direction target."""
        if not self.is_trained:
            raise ValueError(
                "ExtraTreesModel is not trained yet. Call train() or load() before predicting."
            )
    ...
    def save(self, path: Path) -> None:
        ...
        logger.info(f"ExtraTreesModel serialized successfully to {path}")
    ...
    def load(self, path: Path) -> None:
        ...
        logger.info(f"ExtraTreesModel deserialized successfully from {path}")
