...
class LightGBMModel(BaseModel):
    """Concrete implementation of BaseModel using lightgbm LGBMClassifier."""
    ...
    def __init__(
        ...
        # colsample_bytree is the standard sklearn api keyword for feature_fraction
    ...
    def train(self, X: pd.DataFrame, y: pd.Series) -> None:
        """Trains the LightGBM classifier on features X and labels y."""
        logger.info(f"Training LightGBMModel: Samples={len(X)}, Features={X.shape[1]}")
        self._feature_names = list(X.columns)
        self.model.fit(X, y)
        self.is_trained = True
        logger.info("LightGBMModel training completed successfully.")
        
    def predict(self, X: pd.DataFrame) -> np.ndarray:
        """Predicts binary price direction target."""
        if not self.is_trained:
            raise ValueError(
                "LightGBMModel is not trained yet. Call train() or load() before predicting."
            )
    ...
    def save(self, path: Path) -> None:
        ...
        logger.info(f"LightGBMModel serialized successfully to {path}")
    ...
    def load(self, path: Path) -> None:
        ...
        logger.info(f"LightGBMModel deserialized successfully from {path}")
